...
:deep(.detail-layout .th-title) {
  font-size: clamp(30px, 4vw, 44px);
  font-weight: 590;
  letter-spacing: -0.055em;
}

:deep(.detail-layout .th-subtitle) {
  max-width: 58ch;
  margin-top: 10px;
  line-height: 1.75;
}

:deep(.detail-layout .el-button--primary) {
  min-height: 44px;
  border-radius: 13px;
  box-shadow: 0 10px 24px rgba(41, 72, 58, 0.16);
}

:deep(.report-detail-page > .detail-head .head-actions .secondary-button:first-child) {
  display: none;
}
...
:deep(.detail-section) {
  margin-top: 28px;
  padding-top: 24px;
  border-top: 1px solid rgba(216, 221, 213, 0.9);
}
...
:deep(.feature-tags .el-tag) {
  min-height: 32px;
  padding: 0 11px;
  border-color: rgba(63, 101, 82, 0.18);
  border-radius: 9px;
...
:deep(.evidence-list article) {
  padding: 18px;
  border: 1px solid rgba(185, 194, 184, 0.66);
  border-radius: 17px;
...
:deep(.evidence-list article:hover) {
  border-color: rgba(63, 101, 82, 0.32);
  box-shadow: var(--th-shadow-sm);
  transform: translateY(-2px);
}

:deep(.evidence-list p) {
  color: var(--th-text-muted);
  font-size: 13px;
  line-height: 1.78;
  text-wrap: pretty;
...
:deep(.el-alert) {
  margin-top: 26px;
  border: 1px solid rgba(139, 112, 64, 0.18);
  border-radius: 14px;
...
:deep(.raw-json) {
  ...
  border-radius: 13px;
  background: rgba(233, 237, 231, 0.46);
}
...
@media (max-width: 760px) {
  ...
  .plan-recommendation{grid-template-columns:48px minmax(0,1fr);padding:20px}.recommendation-icon{width:46px;height:46px}.recommendation-button{grid-column:1/-1;width:100%}
}
</style>
